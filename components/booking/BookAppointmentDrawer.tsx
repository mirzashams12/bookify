...
import { X, User, Search, UserPlus, Plus, Minus, Stethoscope, ClipboardList } from 'lucide-react';
...
export default function BookAppointmentDrawer({
    ...
    // Derived: Specialties this specific provider offers
    const providerSpecialtyIds = selectedProvider?.provider_specialties?.map(
        (ps: any) => ps.specialty?.id // Access the nested specialty object id
    ) || [];
    const availableSpecialties = specialties.filter(s => providerSpecialtyIds.includes(s.id));

    // Derived: Services for the chosen specialty
    ...
    const adjustDuration = (amount: number) => {
        ...
        const tier = selectedService.rates_chart?.find((r: any) => Number(r.duration_minutes) === newDuration);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (!selectedClient || !selectedService || !selectedProvider) return;
                ...
                <div className="w-full max-w-xl h-full bg-white shadow-2xl flex flex-col">
                    ...
                    <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        <div>
                            ...
                            <p className="text-xs font-bold text-slate-400 uppercase mt-1">Provider-Led Scheduling</p>
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 no-scrollbar space-y-10">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {allProviders.map(p => (
                                    <button
                                        key={p.id}
                                        type="button"
                                        onClick={() => { setSelectedProvider(p); setSelectedSpecialtyId(null); setSelectedService(null); }}
                                        className={`p-4 rounded-2xl border text-left transition-all ${selectedProvider?.id === p.id ? "border-indigo-600 bg-indigo-50 ring-1 ring-indigo-600" : "bg-slate-50 border-slate-100"}`}
                                    >
                                        <p className="font-black text-xs uppercase text-slate-900">{p.fullname}</p>
                                        <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">Lic: {p.license_number}</p>
                                    </button>
                                ))}
                            </div>
                        </section>

                        {/* 3. Specialty Selection (Filtered by Provider) */}
                        {selectedProvider && (
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {availableSpecialties.map(spec => (
                                        <button
                                            ...
                                            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${selectedSpecialtyId === spec.id ? "bg-indigo-600 text-white" : "bg-slate-50 text-slate-400"}`}
                                        >
                                            {spec.name}
                                        </button>
                                    ))}
                                    ...
                                    {availableServices.map((def: any) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`p-5 rounded-[28px] border transition-all cursor-pointer ${isSelected ? "border-indigo-600 bg-indigo-50 ring-1 ring-indigo-600" : "border-slate-100 bg-slate-50"}`}
                                            >
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    {isSelected && hasRates && (
                                                        <div className="flex items-center bg-white border border-indigo-100 rounded-2xl p-1 shadow-sm">
                                                            <button type="button" disabled={!canDecrement} onClick={(e) => { e.stopPropagation(); adjustDuration(-30); }} className="p-2 disabled:opacity-20 text-indigo-600 rounded-xl hover:bg-indigo-50"><Minus size={16} /></button>
                                                            ...
                                                            <button type="button" disabled={!canIncrement} onClick={(e) => { e.stopPropagation(); adjustDuration(30); }} className="p-2 disabled:opacity-20 text-indigo-600 rounded-xl hover:bg-indigo-50"><Plus size={16} /></button>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                        ...
                        <button
                            disabled={loading || !selectedClient || !selectedService || !selectedProvider}
